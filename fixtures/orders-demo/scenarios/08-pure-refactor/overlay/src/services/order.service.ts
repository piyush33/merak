...
export class OrderService {
  ...
  async cancelOrder(user: User, orderId: string): Promise<void> {
    ...
    if (order.status !== OrderStatus.PENDING) {
      throw new InvalidStateError("only pending orders can be cancelled");
    }
    order.status = OrderStatus.CANCELLED;
    await this.orders.update(order);
    await this.releaseAndRefund(order);
    this.events.emit("OrderCancelled", { orderId: order.id });
  }

  private async releaseAndRefund(order: Order): Promise<void> {
